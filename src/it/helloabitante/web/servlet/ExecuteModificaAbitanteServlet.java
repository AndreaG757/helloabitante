...
public class ExecuteModificaAbitanteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (nomeParameter != null && !nomeParameter.isEmpty() &&
			...
			!codiceFiscaleParameter.isEmpty() && etaParameter != null && !etaParameter.isEmpty() && etaParameter.matches("[0-9]+") &&
			mottoParameter != null && !mottoParameter.isEmpty()) {
			
			
			try {
				MyServiceFactory.getAbitanteServiceInstance().aggiorna(new Abitante(Long.parseLong(idParameter), nomeParameter, cognomeParameter, codiceFiscaleParameter,
						Integer.parseInt(etaParameter), mottoParameter));
